    ...
    api = taMegaMenu = {
        ...
        initActions: function () {
            ...
            api.$modal
                ...
                .on('change', '#tamm-panel-icon .tamm-icon-type select', this.switchIconType)
        ...
        switchIconType: function () {
            var field = $(this).closest('.tamm-icon-type');

            field.siblings().hide();
            if($(this).val() == 'svg') {
                field.siblings('.tamm-icon-' + $(this).val() + '').show();
            } else {
                field.siblings('.tamm-icon-font').show();
            }
        ...
        initBackgroundFields: function () {
            ...
            api.$modal.find('.background-text-color-picker').wpColorPicker();
        ...
        getItemData: function (menuItem) {
            var $menuItem = $(menuItem).closest('li.menu-item'),
                ...
                $menuSVG = $menuItem.find('.tamm-data-svg'),
            ...
            if (!_.isEmpty(children)) {
                _.each(children, function (item) {
                    var $item = $(item),
                        ...
                        $menuSVG = $item.find('.tamm-data-svg'),
