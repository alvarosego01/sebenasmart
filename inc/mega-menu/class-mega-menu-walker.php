...
class Martfury_Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $item_mega_background ) {
			...
			if ( isset( $item_mega_background['textcolor'] ) && ! empty( $item_mega_background['textcolor'] ) ) {
				$inline .= '; color:' . esc_attr( $item_mega_background['textcolor'] );
			}
		...
		if ( in_array( 'menu-item-has-children', $classes ) ) {
			...
			if ( ! $depth && intval( $this->in_mega ) ) {
				...
				if ( $item_mega_background && isset( $item_mega_background['textcolor'] ) && ! empty( $item_mega_background['textcolor'] ) ) {
					$classes[] = 'custom-text-color';
				}
			}
		...
		if( $item_icon_type == 'svg' ) {
			$icon_output  = ! empty( $item_icon_svg ) ? '<span ' . $item_icon_color . ' class="mf-svg-icon">' . martfury_sanitize_svg( $item_icon_svg ) . '</span> ' : '';
		} else {
